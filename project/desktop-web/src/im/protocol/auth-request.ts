// automatically generated by the FlatBuffers compiler, do not modify

/* eslint-disable @typescript-eslint/no-unused-vars, @typescript-eslint/no-explicit-any, @typescript-eslint/no-non-null-assertion */

import * as flatbuffers from 'flatbuffers';

import { Platform } from '../../im/protocol/platform.js';


export class AuthRequest {
  bb: flatbuffers.ByteBuffer|null = null;
  bb_pos = 0;
  __init(i:number, bb:flatbuffers.ByteBuffer):AuthRequest {
  this.bb_pos = i;
  this.bb = bb;
  return this;
}

static getRootAsAuthRequest(bb:flatbuffers.ByteBuffer, obj?:AuthRequest):AuthRequest {
  return (obj || new AuthRequest()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

static getSizePrefixedRootAsAuthRequest(bb:flatbuffers.ByteBuffer, obj?:AuthRequest):AuthRequest {
  bb.setPosition(bb.position() + flatbuffers.SIZE_PREFIX_LENGTH);
  return (obj || new AuthRequest()).__init(bb.readInt32(bb.position()) + bb.position(), bb);
}

token():string|null
token(optionalEncoding:flatbuffers.Encoding):string|Uint8Array|null
token(optionalEncoding?:any):string|Uint8Array|null {
  const offset = this.bb!.__offset(this.bb_pos, 4);
  return offset ? this.bb!.__string(this.bb_pos + offset, optionalEncoding) : null;
}

deviceId():string|null
deviceId(optionalEncoding:flatbuffers.Encoding):string|Uint8Array|null
deviceId(optionalEncoding?:any):string|Uint8Array|null {
  const offset = this.bb!.__offset(this.bb_pos, 6);
  return offset ? this.bb!.__string(this.bb_pos + offset, optionalEncoding) : null;
}

platform():Platform {
  const offset = this.bb!.__offset(this.bb_pos, 8);
  return offset ? this.bb!.readInt8(this.bb_pos + offset) : Platform.UNKNOWN;
}

appVersion():string|null
appVersion(optionalEncoding:flatbuffers.Encoding):string|Uint8Array|null
appVersion(optionalEncoding?:any):string|Uint8Array|null {
  const offset = this.bb!.__offset(this.bb_pos, 10);
  return offset ? this.bb!.__string(this.bb_pos + offset, optionalEncoding) : null;
}

static startAuthRequest(builder:flatbuffers.Builder) {
  builder.startObject(4);
}

static addToken(builder:flatbuffers.Builder, tokenOffset:flatbuffers.Offset) {
  builder.addFieldOffset(0, tokenOffset, 0);
}

static addDeviceId(builder:flatbuffers.Builder, deviceIdOffset:flatbuffers.Offset) {
  builder.addFieldOffset(1, deviceIdOffset, 0);
}

static addPlatform(builder:flatbuffers.Builder, platform:Platform) {
  builder.addFieldInt8(2, platform, Platform.UNKNOWN);
}

static addAppVersion(builder:flatbuffers.Builder, appVersionOffset:flatbuffers.Offset) {
  builder.addFieldOffset(3, appVersionOffset, 0);
}

static endAuthRequest(builder:flatbuffers.Builder):flatbuffers.Offset {
  const offset = builder.endObject();
  return offset;
}

static createAuthRequest(builder:flatbuffers.Builder, tokenOffset:flatbuffers.Offset, deviceIdOffset:flatbuffers.Offset, platform:Platform, appVersionOffset:flatbuffers.Offset):flatbuffers.Offset {
  AuthRequest.startAuthRequest(builder);
  AuthRequest.addToken(builder, tokenOffset);
  AuthRequest.addDeviceId(builder, deviceIdOffset);
  AuthRequest.addPlatform(builder, platform);
  AuthRequest.addAppVersion(builder, appVersionOffset);
  return AuthRequest.endAuthRequest(builder);
}
}
