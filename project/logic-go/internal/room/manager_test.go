...
func TestRoomManagerTriggerSnapshotEnqueuesRoomID(t *testing.T) {
	m := &RoomManager{
...
func TestRoomManagerTriggerSnapshotIgnoresEmptyRoomID(t *testing.T) {
	m := &RoomManager{
		snapshotChan: make(chan string, 1),
		logger:       slog.New(slog.NewTextHandler(io.Discard, nil)),
	}
...
func TestRoomManagerTriggerSnapshotDropsWhenQueueFull(t *testing.T) {
	m := &RoomManager{
		snapshotChan: make(chan string, 1),
		logger:       slog.New(slog.NewTextHandler(io.Discard, nil)),
	}
