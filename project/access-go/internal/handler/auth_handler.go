...
func (h *Handler) HandleFirstStream(ctx context.Context, conn *connection.Connection, stream *webtransport.Stream) error {
	...
	if err != nil {
		if errors.Is(err, ErrFrameTooLarge) {
			h.logger.Warn("Reject first frame because body is too large", "conn_id", conn.ID(), "frameType", frameType, "error", err)
		}
	...
	if frameType != FrameTypeAuth {
		...
		h.sendClientResponse(conn, "", im_protocol.ErrorCodeAUTH_FAILED, "auth required", im_protocol.ResponsePayloadNONE, nil)
...
func (h *Handler) handleAuth(ctx context.Context, conn *connection.Connection, stream *webtransport.Stream, body []byte) error {
	...
	if err != nil {
		...
		h.sendClientResponse(conn, "", im_protocol.ErrorCodeUNKNOWN_ERROR, "internal error", im_protocol.ResponsePayloadNONE, nil)
	...
	if userInfo == nil {
		...
		h.sendClientResponse(conn, "", im_protocol.ErrorCodeAUTH_FAILED, "invalid token", im_protocol.ResponsePayloadNONE, nil)
	...
	if userInfo.DeviceID != deviceID {
		...
		h.sendClientResponse(conn, "", im_protocol.ErrorCodeAUTH_FAILED, "device mismatch", im_protocol.ResponsePayloadNONE, nil)
	...
	if !strings.EqualFold(userInfo.Platform, platform.String()) {
		...
		h.sendClientResponse(conn, "", im_protocol.ErrorCodeAUTH_FAILED, "platform mismatch", im_protocol.ResponsePayloadNONE, nil)
	...
	if !isCurrent {
		...
		h.sendClientResponse(conn, "", im_protocol.ErrorCodeAUTH_FAILED, "token expired or replaced", im_protocol.ResponsePayloadNONE, nil)
	...
	// 发送认证成功响应
	h.sendClientResponse(conn, "", im_protocol.ErrorCodeSUCCESS, "success", im_protocol.ResponsePayloadNONE, nil)
