...
**最高准则：性能与低时延优先**。IM 应用必须把实时性作为第一优先级，所有设计、实现和优化建议都要优先保证最低时延和最高吞吐。可靠性、可扩展性、持久化、安全、观测等改造都不能牺牲 Access/Logic/Web 消息主链路的低延迟，禁止在主链路中引入阻塞等待、长事务、无界排队或高延迟同步调用。

- 优先选择非阻塞、异步、批处理、少拷贝、短路径、有界队列和快速失败设计
- 可靠性增强应优先使用异步确认、轻量级 outbox、幂等去重、后台补偿，避免让客户端发送路径等待慢速存储或跨服务长调用
- 背压策略必须快速失败或降级，并且可观测、可度量，禁止无限等待
- 优化建议必须说明对 P50/P95/P99 时延、吞吐、队列积压和资源占用的影响
- 房间、游戏、群消息扩散等高扇出场景必须采用分片、就近路由、批量拉取和并发上限，避免热点拖慢全局消息处理

1. **非阻塞要求**: 在 access、logic、web 这三个模块中，绝对不允许阻塞消息处理，只需要打印日志即可。如果修改或生成的代码阻塞了消息处理，必须立即通知用户
